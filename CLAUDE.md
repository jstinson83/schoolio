...
- **Adding a second `com.google.cloud:*`/`io.grpc:*`-using dependency can silently break every *other* gRPC-based client in the fat jar, with an error that names neither dependency.** Hit for real: adding `google-cloud-storage` (for email attachments) broke `FirestoreNotificationStateStore`'s Firestore reads with `io.grpc.StatusRuntimeException: INTERNAL: Could not find policy 'pick_first'` — a message with nothing pointing at Storage or Firestore in it. Root cause: several `grpc-*` jars (`grpc-core`, `grpc-util`, `grpc-services`, `grpc-grpclb`, `grpc-xds`, `grpc-rls`, ...) each ship their *own* file at the identical path `META-INF/services/io.grpc.LoadBalancerProvider`, `./gradlew buildFatJar` (this project's plain Ktor fat-jar task, not Shadow) does **not** merge same-path resources across jars — it just keeps one and silently drops the rest — and `grpc-core`'s file (the one listing `PickFirstLoadBalancerProvider`, which every default gRPC channel needs) happened to be the one kept before this dependency addition and the one *dropped* after, purely because adding a new dependency changed the jar processing order. This isn't Storage-specific or Firestore-specific — it's a latent landmine any future `grpc-*`-pulling dependency can retrigger, and it will always manifest as a runtime gRPC failure in some *unrelated* client, never a build error. **To check**: `unzip -p build/libs/*-all.jar META-INF/services/io.grpc.LoadBalancerProvider` after `./gradlew buildFatJar` — if it doesn't include `io.grpc.internal.PickFirstLoadBalancerProvider`, this bug is back. Fixed here by excluding `google-cloud-storage`'s optional gRPC transport (`grpc-google-cloud-storage-v2`/`gapic-google-cloud-storage-v2` and everything they pull in — `grpc-xds`, `grpc-alts`, `grpc-googleapis`, `grpc-grpclb`, `backend/build.gradle.kts`), which this app never uses anyway (`GcsAttachmentStore` only calls the plain HTTP/JSON Storage API via `StorageOptions.getDefaultInstance()`), restoring the same jar-processing order that happened to work before. This is a real fix for *this* addition, not a general one — a future dependency bump could shuffle the ordering again and drop a different provider; if `pick_first` (or any other gRPC "Could not find policy" error) resurfaces, re-run the check above before assuming the new code is at fault.
